...
export const getLocalizedRiders = (t: any) => {
    return {
        CANCER: ['암진단비', '암수술비', '항암치료비', '항암방사선약물치료', '표적항암약물허가치료'],
        PSEUDO: ['유사암진단비', '제자리암진단비', '경계성종양진단비', '소액암진단비'],
        BRAIN: ['뇌혈관질환진단', '뇌혈관질환수술', '뇌졸중진단'],
        HEART: ['허혈성심장질환진단', '허혈성심장질환수술', '심혈관질환진단(부정맥포함)', '급성심근경색진단'],
        INJURY: ['상해수술비', '골절진단비', '상해후유장해', '골절수술비', '5대골절진단'],
        DISEASE: ['질병수술비', '1-5종수술비', '질병후유장해', '희귀난치성질환진단']
    }
}

export const getGroupedDiseaseData = (t?: any, locale: string = 'ko'): TopCategory[] => {
    // A simplified t function fallback
    const fallbackT = (key: string) => key;
    const translation = t || fallbackT;
    const riders = getLocalizedRiders(translation);
            ...
            subCategories: [
                {
                    ...
                    items: [
                        ...
                        { name: isKo ? '기관/기관지 및 폐' : 'Trachea, bronchus & lung', code: 'D02.2', riders: riders.PSEUDO, deepAnalysis: isKo ? '간유리음영(GGO) 제자리암은 추적 관찰이 필요합니다.' : 'Ground-glass opacity (GGO) showing CIS features requires close monitoring; check for "Clinical CIS" possibilities.' },
        ...
        },
        {
            ...
            subCategories: [
                ...
                },
                {
                    ...
                    items: [
                        { name: isKo ? '무릎 전방십자인대 파열' : 'ACL tear', code: 'S83.5', riders: ['상해후유장해'], isImportant: true, claimTips: isKo ? '단순 파열보다 수술 후 "동요(흔들림)" 정도가 중요합니다. KT-2000 검사 등으로 5mm 이상의 동요를 입증하여 상해후유장해 보험금을 확보하십시오.' : 'Prove 5mm+ instability via KT-2000 for disability benefits.', deepAnalysis: isKo ? '동요 측정 결과 5mm/10mm/15mm 기준에 따라 지급률 5%/10%/20%가 결정되는 고액 장해 건입니다.' : 'Instability measurements (5/10/15mm) determine 5/10/20% disability rates.' },
                        { name: isKo ? '아킬레스건 파열' : 'Achilles tendon rupture', code: 'S86.0', riders: ['상해수술비'], claimTips: isKo ? '아킬레스건 파열은 외상 기여도가 100%인 경우가 많습니다.' : 'Achilles rupture is often 100% traumatic; ensure full benefit payout.' }
                    ]
                }
            ]
        },
